...
//
// Extracted from CreateEndpointModal (plan 23) so the modal and the inline
// "first mock" onboarding form (a plain, unmodal rendering of this same
// component) are one implementation, not two forms that can drift apart.
export function EndpointForm({
  ...
  slug: string;
  onCreated: (method: string, path: string) => void;
  submitLabel?: string;
}) {
  ...
  async function create() {
    ...
    try {
      ...
      onCreated(method, effectivePath);
    } catch (e) {
